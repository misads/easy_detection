...
def focal_loss(logits, targets, alpha, gamma, normalizer):
    ...
    modulator = torch.exp(gamma * targets.float() * neg_logits - gamma * torch.log1p(torch.exp(neg_logits)))
...
class DetectionLoss(nn.Module):
    ...
    def forward(self, cls_outputs, box_outputs, cls_targets, box_targets, num_positives):
        ...
        for l in range(levels):
            ...
            if legacy_focal:
                cls_loss = focal_loss(
                    ...
                    alpha=self.alpha, gamma=self.gamma, normalizer=num_positives_sum)
            else:
                cls_loss = new_focal_loss(
                    cls_outputs_at_level, cls_targets_at_level_oh,
                    alpha=self.alpha, gamma=self.gamma, normalizer=num_positives_sum, label_smoothing=label_smoothing)
        ...
        #     cls_targets_non_neg_cal=cls_targets_at_level+1
        #     # cls_targets_non_neg = cls_targets_at_level+1
        #     # pdb.set_trace()
        #     cls_targets_at_level_oh = F.one_hot(cls_targets_at_level * cls_targets_non_neg_cal, self.num_classes)
