"""
Implements anchors (with octaves)
...
class AnchorGenerator(nn.Module):
    def __init__(
        self, scales=(32, 64, 128, 256, 512), aspect_ratios=(0.5, 1.0, 2.0), octaves=None):
        super(AnchorGenerator, self).__init__()

        assert isinstance(octaves, (tuple, list, type(None)))
        if octaves is not None:
            scales = [[scale * octave for octave in octaves] for scale in scales]
    ...
    def generate_base_anchors(scale, aspect_ratios, device="cpu"):
        ...
        octave_scale = len(scale.shape) == 1
        if octave_scale:
            scale = scale[None, :]
            aspect_ratios = aspect_ratios[:, None]
